...
touch WebLog.csv #Final output log
...
# Ready to make .csv output      
      ...
      #cat Finallogtranstart1.csv |  awk -F"[,]+" '{print $1,$3,$7}' >> WebLog.csv
      awk -F '|' '!x[$1,$2,$3,$4]++' Finallogdone.csv > filename_dedup.csv
      cat filename_dedup.csv |  awk -F"[,]+" '{print $4,$5,$7}' >> WebLog.csv
      cat Finallogtranend1.csv |  awk -F"[,]+" '{print $1,$3,$7}' >> WebLog.csv
      sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" WebLog.csv
     done
